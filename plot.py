...
#sel = data[:,3]+data[:,8] < (data[:,3]+data[:,8]).max()/2.0
#data = data[sel,:]
...
ax.plot(xa,xa*0.0+1.0,'k--',lw=2,label=r"True ${\rm B}_\mathcal{AB}$")
ax.plot(xa,data[:,3]+data[:,10],'C1-',lw=2,label=r"Projected $\langle{\rm B}_\mathcal{AB}\rangle+\sigma_+-\sigma_-$")
ax.plot(xa,data[:,3],'k-',lw=2,label=r"Estimated $\langle{\rm B}_\mathcal{AB}\rangle$")
