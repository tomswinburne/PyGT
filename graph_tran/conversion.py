...
def dump_gephi_format(mindatafile, tsdatafile, suffix):
    """Create gephi-style csv format files from a min.data and ts.data file."""
    mindata = np.loadtxt(mindatafile)
    nnodes = mindata.shape[0]
    node_df = pd.DataFrame(columns=['Id', 'Energy'])
    node_df['Id'] = np.arange(1, nnodes+1, 1)
    node_df['Energy'] = mindata[:, 0].astype('float')
    node_df.to_csv(f'csvs/node_list_{suffix}.csv', index=False)
    tsdata = np.loadtxt(tsdatafile)
    edge_df = pd.DataFrame(columns=['source', 'target', 'tsenergy'])
    edge_df['source'] = tsdata[:, 3].astype('int')
    edge_df['target'] = tsdata[:, 4].astype('int')
    edge_df['tsenergy'] = tsdata[:, 0].astype('float')
