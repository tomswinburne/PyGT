...
in blocks, or one at a time. For use with the `PyGT.io` module,
...
simulatenously. [Swinburne20]_ In practice, the larger the block of nodes, the less
...
def GT(rm_vec,B,tau,block=20,order=None,rates=False,Ndense=50,screen=False):

	r"""
	Main function for GT code.
	...
	rates: 	bool, optional
			Whether to return the GT-reduced rate matrix in addition to B and tau. Default = False
...
def singleGT(rm_vec,B,tau):
	...
	case, if a `LinAlgError` this is fed back to the main GT routine to allow
	for one-by-one node removal for that block.
	...
	success: bool
			 False if `LinAlgError` was raised
